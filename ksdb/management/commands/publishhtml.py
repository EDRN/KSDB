...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if 'protocol' in options['rdftype']:
            rdf = self.getprotocolhtml(filterobj, options['filterval'])
    ...
    def getpublicationrdf(self, filterobj, filterval):
        ...
        if pubs:
            for pub in pubs:
                ...
                if pub.journal:
                    self._graph.add( (pubi, self._schema.journal, Literal(pub.journal.split(":")[0].strip())) )
                if pub.title:
    ...
    def getprotocolhtml(self, filterobj, filterval):
        prots = None
        protocol_header = ["ID","Title","Abstract","Investigator(s)"]
        self._html += "<table class='listing' ><thead><tr><th>"+"</th><th>".join(protocol_header)+"</th></thead></tr><tbody>"
        if filterobj is program:
